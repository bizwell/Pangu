 ...
 * @author siuming
 */
public abstract class AbstractFastDfsClient implements Closeable
{
    ...
    protected AbstractFastDfsClient(Builder builder)
    {
    ...
    public CompletableFuture<FileId> upload(String group, File file)
    {
        ...
        return trackerClient.uploadStorageGet(group).thenCompose(server -> storageClient.upload(server, file));
    }
    ...
    public CompletableFuture<FileId> upload(String group, Object content, String filename, long size)
    {
        ...
        return trackerClient.uploadStorageGet(group).thenCompose(server -> storageClient.upload(server, content, filename, size));
    }
    ...
    {
        Objects.requireNonNull(file, "file must not be null.");
        return trackerClient.uploadStorageGet(group).thenCompose(server -> storageClient.uploadAppender(server, file));
    }
    ...
    public CompletableFuture<FileId> uploadAppender(String group, Object content, String filename, long size)
    {
        Objects.requireNonNull(content, "content must not be null.");
        Objects.requireNonNull(filename, "filename must not be null.");
        return trackerClient.uploadStorageGet(group).thenCompose(server -> storageClient.uploadAppender(server, content, filename, size));
    }
    ...
    public CompletableFuture<Void> download(FileId fileId, Object out)
    {
        ...
        return trackerClient.downloadStorageGet(fileId).thenCompose(server -> storageClient.download(server, fileId, out));
    }
    ...
    public CompletableFuture<Void> download(FileId fileId, Object out, int offset, int size)
    {
        Objects.requireNonNull(fileId, "fileId must not be null.");
        Objects.requireNonNull(out, "out must not be null.");
        return trackerClient.downloadStorageGet(fileId).thenCompose(server -> storageClient.download(server, fileId, out, offset, size));
    }
    ...
    {
        Objects.requireNonNull(fileId, "fileId must not be null.");
        return trackerClient.updateStorageGet(fileId).thenCompose(server -> storageClient.delete(server, fileId));
    }
    ...
    public CompletableFuture<Void> append(FileId fileId, File file)
    {
        Objects.requireNonNull(fileId, "fileId must not be null.");
        Objects.requireNonNull(file, "file must not be null.");
        return trackerClient.updateStorageGet(fileId).thenCompose(server -> storageClient.append(server, fileId, file));
    }
    ...
    public CompletableFuture<Void> append(FileId fileId, Object content, long size)
    {
        Objects.requireNonNull(fileId, "fileId must not be null.");
        Objects.requireNonNull(content, "content must not be null.");
        return trackerClient.updateStorageGet(fileId).thenCompose(server -> storageClient.append(server, fileId, content, size));
    }
    ...
    public CompletableFuture<Void> modify(FileId fileId, File file, int offset)
    {
        Objects.requireNonNull(fileId, "fileId must not be null.");
        Objects.requireNonNull(file, "file must not be null.");
        return trackerClient.updateStorageGet(fileId).thenCompose(server -> storageClient.modify(server, fileId, file, offset));
    }
    ...
    public CompletableFuture<Void> modify(FileId fileId, Object content, long size, int offset)
    {
        Objects.requireNonNull(fileId, "fileId must not be null.");
        Objects.requireNonNull(content, "content must not be null.");
        return trackerClient.updateStorageGet(fileId).thenCompose(server -> storageClient.modify(server, fileId, content, size, offset));
    }
    ...
    {
        Objects.requireNonNull(fileId, "fileId must not be null.");
        return trackerClient.updateStorageGet(fileId).thenCompose(server -> storageClient.truncate(server, fileId, truncatedSize));
    }
    ...
    public CompletableFuture<Void> metadataSet(FileId fileId, FileMetadata metadata, byte flag)
    {
        Objects.requireNonNull(fileId, "fileId must not be null.");
        Objects.requireNonNull(metadata, "metadata must not be null.");
        return trackerClient.updateStorageGet(fileId).thenCompose(server -> storageClient.setMetadata(server, fileId, metadata, flag));
    }
    ...
    {
        Objects.requireNonNull(fileId, "fileId must not be null.");
        return trackerClient.updateStorageGet(fileId).thenCompose(server -> storageClient.getMetadata(server, fileId));
    }
    ...
    {
        Objects.requireNonNull(fileId, "fileId must not be null.");
        return trackerClient.updateStorageGet(fileId).thenCompose(server -> storageClient.getInfo(server, fileId));
    }
