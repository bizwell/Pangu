...
public class PahoClient
{
    ...
    /**
     * 发送消息<br />
     * <strong>序列化方式为 JSON 序列化，QOS 为 0（最多发一次）</strong>
     * 
     * @param topic 主题
    ...
    public <T extends Serializable> void sendJson(String topic, T... message) throws MqttPersistenceException, MqttException
    {
        ...
        for(T item: message)
        {
            byte bytes[] = StringUtil.toBytes(JsonUtil.toJSON(item));
            MqttMessage msg = new MqttMessage();
            msg.setPayload(bytes);
            msg.setQos(0);
            client.publish(topic, msg);

            log.info("向 {} 发送 JSON 序列化消息, 长度: {}", topic, bytes.length);
     ...
     * @throws MqttPersistenceException
     */
    public <T extends Serializable> void send(String topic, String... message) throws MqttPersistenceException, MqttException
    {
        if(ArrayUtil.isEmpty(message))
        {
            log.warn("发送到主题 {} 的消息为空", topic);
        ...
        {
            byte bytes[] = StringUtil.toBytes(item);
            MqttMessage msg = new MqttMessage();
            msg.setPayload(bytes);
            msg.setQos(0);
            client.publish(topic, msg);

            log.info("向 {} 发送字符串消息, 长度: {}", topic, bytes.length);
            log.debug("消息内容是：{}", item);
        }
    }
