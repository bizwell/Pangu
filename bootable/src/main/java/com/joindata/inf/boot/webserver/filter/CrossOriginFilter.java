 ...
 * </p>
 * <p>
 * A typical example is to use this filter to allow cross-domain <a href="http://cometd.org">cometd</a> communication using the standard long polling transport instead of the JSONP transport (that is less efficient and less reactive to failures).
 * </p>
 * <p>
 * This filter allows the following configuration parameters:
 * <ul>
 * <li><b>allowedOrigins</b>, a comma separated list of origins that are allowed to access the resources. Default value is <b>*</b>, meaning all origins.<br />
 * If an allowed origin contains one or more * characters (for example http://*.domain.com), then "*" characters are converted to ".*", "." characters are escaped to "\." and the resulting allowed origin interpreted as a regular expression.<br />
 * Allowed origins can therefore be more complex expressions such as https?://*.domain.[a-z]{3} that matches http or https, multiple subdomains and any 3 letter top-level domain (.com, .net, .org, etc.).</li>
 * <li><b>allowedMethods</b>, a comma separated list of HTTP methods that are allowed to be used when accessing the resources. Default value is <b>GET,POST,HEAD</b></li>
 * <li><b>allowedHeaders</b>, a comma separated list of HTTP headers that are allowed to be specified when accessing the resources. Default value is <b>X-Requested-With,Content-Type,Accept,Origin</b>. If the value is a single "*", this means that any headers will be accepted.</li>
 * <li><b>preflightMaxAge</b>, the number of seconds that preflight requests can be cached by the client. Default value is <b>1800</b> seconds, or 30 minutes</li>
 * <li><b>allowCredentials</b>, a boolean indicating if the resource allows requests with credentials. Default value is <b>false</b></li>
 * <li><b>exposedHeaders</b>, a comma separated list of HTTP headers that are allowed to be exposed on the client. Default value is the <b>empty list</b></li>
 * <li><b>chainPreflight</b>, if true preflight requests are chained to their target resource for normal handling (as an OPTION request). Otherwise the filter will response to the preflight. Default is true.</li>
 * </ul>
 * </p>
 * <p>
...
public class CrossOriginFilter implements Filter
{
    ...
    public void init(FilterConfig config) throws ServletException
    {
        ...
        {
            LOG.debug("Cross-origin filter configuration: " + ALLOWED_ORIGINS_PARAM + " = " + allowedOriginsConfig + ", " + ALLOWED_METHODS_PARAM + " = " + allowedMethodsConfig + ", " + ALLOWED_HEADERS_PARAM + " = " + allowedHeadersConfig + ", " + PREFLIGHT_MAX_AGE_PARAM + " = " + preflightMaxAgeConfig + ", " + ALLOW_CREDENTIALS_PARAM + " = " + allowedCredentialsConfig + "," + EXPOSED_HEADERS_PARAM + " = " + exposedHeadersConfig + "," + CHAIN_PREFLIGHT_PARAM + " = " + chainPreflightConfig);
        }
    }
    ...
    private void handleSimpleResponse(HttpServletRequest request, HttpServletResponse response, String origin)
    {
        ...
        // XXX 不知道是有 BUG 还是怎么着，执行简单响应时，没有下面这几句
